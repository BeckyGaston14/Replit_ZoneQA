Warning: truncated output (original token count: 94191)
Total output lines: 7258
...
async def restore_resource(resource: str, id: str, user=Depends(require_roles("admin", "qa_manager"))):
    ...
    updated = await db[resource].find_one_and_update({"id": id}, {"$set": {
        ...
        "updated_at": now_iso(),
    }, "$unset": {"archived_at": "", "archived_by": "", "a…34191 tokens truncated…  stale_gold_tests = [{"testcase_id": tid, "name": tcs.get(tid, {}).get("name", "?"), "stale_evidence": stale_map[tid]}
...
async def analytics_coverage(user=Depends(get_current_user)):
    ...
    munis = [m for m in await crud_list("municipalities") if m.get("id") in used_municipality_ids or not m.get("sample_data")]
    cfg = await db.config.find_one({"id": "global"}, {"_id": 0}) or {}
