...
def _comparison_finding_documents(body, testcase_id, user, timestamp):
    ...
    for value in values if isinstance(values, list) else []:
        ...
        documents.append({
            ...
            "finding_type_detail": str(value.get("finding_type_detail") or ""),
...
def _bassett_finding_document(body, testcase_id, user, timestamp):
    ...
    return {
        ...
        "finding_type_detail": str(finding.get("finding_type_detail") or ""),
    ...
    *,
    ...
    if create_finding:
        ...
        finding = {
            ...
            "finding_type_detail": str(finding_input.get("finding_type_detail") or ""),
...
async def bassett_convert_to_finding(id: str, body: Dict[str, Any] = None, user=Depends(get_current_user)):
    ...
    finding = {
        ...
        "finding_type_detail": str(body.get("finding_type_detail") or ""),
        "severity": finding_severity, "criticality": finding_criticality,
    ...
    for run in _canonical_bassett_lineages(issues, executions, active_scenario_ids=set(scenario_by_id)):
        ...
        candidate = {
            **run, **authoritative,
            ...
            "final_result": _canonical_bassett_result(run.get("result")),
...
async def analytics_coverage(user=Depends(get_current_user), scope: str = "both"):
    ...
    # A configured bucket with no active tests is "Not Represented", not a
    # coverage gap. Only represented populations can be uncovered.
    muni_gaps = [m for m in municipalities if m["tests"] > 0 and m["evaluated"] == 0]
    cat_gaps = [c for c in categories if c["tests"] > 0 and c["evaluated"] == 0]
    crit_gaps = [c for c in criticality if c["tests"] > 0 and c["evaluated"] == 0]
