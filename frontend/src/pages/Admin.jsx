      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="users">
          ...
          <div className="bg-card border rounded-xl max-w-full overflow-x-auto overscroll-x-contain"><table className="w-full min-w-[720px] text-sm">
            <thead className="bg-[var(--paper)] text-left"><tr>{USER_COLUMNS.map((column) => <SortableTableHeader key={column.key} column={column} sort={userSort} onSort={(key) => setUserSort((current) => nextSort(current, key))} />)}<th className="px-4 py-2 text-right text-[11px] uppercase tracking-wide text-muted-foreground">Actions</th></tr></thead>
